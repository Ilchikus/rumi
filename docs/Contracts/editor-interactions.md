...
| Browse database table views | Open a database page or an embedded database table | Full-page and embedded database tables use the editor content width and scroll oversized rows or columns inside a shared two-axis frame. The frame has no outer border, top/bottom section separators, final-row bottom rule, or manual Refresh control; loading remains application-owned. |
